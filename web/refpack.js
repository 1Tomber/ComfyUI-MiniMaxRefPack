...
function draw(node) {
    ...
    for (const row of CANVAS_ROWS.rows) {
        ...
        // The insertion caret: a bright bar in the gap the tile would land in. Drawn from
        // the same tile regions the drop index is computed from, so what is shown and what
        // happens cannot disagree.
        if (drag && drag.active && drag.kind === kind && typeof drag.insertAt === "number") {
            const at = drag.insertAt;
            const anchor = arr.length
                ? (at >= arr.length
                    ? { x: CL.x0 + (arr.length - 1) * (CL.tile + CL.gap) + CL.tile, y: tileY }
                    : { x: CL.x0 + at * (CL.tile + CL.gap) - CL.gap, y: tileY })
                : { x: CL.x0, y: tileY };
            const cx = Math.round(anchor.x + CL.gap / 2) + 0.5;
            ctx.strokeStyle = C.danger;
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.moveTo(cx, anchor.y - 2);
            ctx.lineTo(cx, anchor.y + CL.tile + 2);
            ctx.stroke();
        }
